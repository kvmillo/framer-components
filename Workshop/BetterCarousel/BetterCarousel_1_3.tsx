...
export default function BetterCarousel(props) {
    ...
    const isLastFullyVisible = React.useCallback(() => {
        try {
            ...
            return lastRect.right <= rightEdge + 1
        } catch (e) {
            return false
        }
    }, [])
    ...
    // Single effect wiring Splide events — removed duplicate that caused every event to fire twice
    React.useEffect(() => {
        if (!splideInstance) return
        updateArrowState()
        recomputeProgress()
    ...
    }, [splideInstance, recomputeProgress, updateArrowState])
