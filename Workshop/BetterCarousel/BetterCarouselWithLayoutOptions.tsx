 ...
 * @framerDisplayName Better Carousel With Layout Display
 */
...
interface CarouselProps {
    ...
    displayMode?: "carousel" | "stack" | "grid"
    // Stack
    stackDirection?: "horizontal" | "vertical"
    stackWrap?: boolean
    stackSizingW?: "fill" | "fit"
    stackSizingH?: "fill" | "fit"
    // Grid
    gridColumns?: number
    gridGapX?: number
    gridGapY?: number
    gridSizingW?: "fill" | "fit"
    gridSizingH?: "fill" | "fit"
    gridCellAlign?: AlignKey
    gap?: number
    type?: string
    dragMode?: string
...
export default function BetterCarouselWithLayoutOptions(props: CarouselProps) {
    const {
        slideFrames,
    ...
    const gridIdRef = React.useRef(`bclo-${Math.random().toString(36).slice(2, 8)}`)
    ...
    // Re-mount Splide when key options change (carousel mode only)
    React.useEffect(() => {
        const win = window as any
        if (displayMode === "carousel" && win.Splide && splideRef.current) mountSplide()
    }, [ // eslint-disable-line react-hooks/exhaustive-deps
        displayMode,
        slideFrames,
        gap,
    ...
    const gridAlignMap: Record<AlignKey, { justifyItems: string; alignItems: string }> = {
    ...
    const renderStack = () => {
        const isRow = stackDirection === "horizontal"
        // cross-axis alignment drives the opposite dimension
        const alignItems = (isRow ? stackSizingH : stackSizingW) === "fill" ? "stretch" : "flex-start"
        return (
            <div
                style={{
                    display: "flex",
                ...
                {(slideFrames || []).map((frame, i) => (
                    <div
                        ...
                        style={{
                            // main-axis sizing
                            ...(isRow
                                ? { flex: stackSizingW === "fill" ? "1 1 0" : "0 0 auto", minWidth: 0 }
                                : { alignSelf: stackSizingH === "fill" ? "stretch" : "auto", width: stackSizingW === "fill" ? "100%" : "auto" }
                            ),
                        }}
                    >
                        {frame}
    ...
    const renderGrid = () => {
        const { justifyItems, alignItems } = gridAlignMap[gridCellAlign] ?? gridAlignMap.topLeft
        const effectiveJustify = gridSizingW === "fill" ? "stretch" : justifyItems
        const effectiveAlign   = gridSizingH === "fill" ? "stretch" : alignItems
        const fillStyles = [
            gridSizingW === "fill" ? `#${gridId} [data-gi] > * { width: 100% !important; box-sizing: border-box !important; }` : "",
            gridSizingH === "fill" ? `#${gridId} [data-gi] > * { height: 100% !important; }` : "",
        ].filter(Boolean).join("\n")
        return (
            <>
                {fillStyles && <style>{fillStyles}</style>}
                <div
                    id={gridId}
                    style={{
                        display: "grid",
                ...
                >
                    {(slideFrames || []).map((frame, i) => (
                        <div
                            ...
                            data-gi
                            style={{
                                width: gridSizingW === "fill" ? "100%" : "auto",
                                height: gridSizingH === "fill" ? "100%" : "auto",
                            }}
                        >
                            {frame}
                        </div>
    ...
    return (
        ...
        >
            {displayMode === "stack" && (slideFrames && slideFrames.length > 0 ? renderStack() : renderPlaceholder())}
            {displayMode === "grid" && (slideFrames && slideFrames.length > 0 ? renderGrid() : renderPlaceholder())}
            {displayMode === "carousel" && slideFrames && slideFrames.length > 0 ? (
                <div
                    className="splide"
                    ref={splideRef}
                    style={{
...
BetterCarouselWithLayoutOptions.defaultProps = {
    slideFrames: [],
    displayMode: "carousel",
    stackDirection: "vertical",
    stackWrap: false,
...
addPropertyControls(BetterCarouselWithLayoutOptions, {
    slideFrames: {
    ...
    displayMode: {
        ...
        optionTitles: ["Carousel", "Stack", "Grid"],
    ...
    stackDirection: {
        ...
        hidden: (props) => props.displayMode !== "stack",
    },
    stackWrap: {
    ...
    gridCellAlign: {
        ...
        hidden: (props) => props.displayMode !== "grid" || (props.gridSizingW === "fill" && props.gridSizingH === "fill"),
    },
    gap: {
        type: ControlType.Number,
        title: "Gap",
    ...
    type: {
        ...
        hidden: (props) => props.displayMode !== "carousel",
    },
    dragMode: {
        type: ControlType.SegmentedEnum,
        title: "Drag",
